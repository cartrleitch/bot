...
def main():
    ...
    def play_record(voice_client):
        """plays record output"""
        print("Attempting to play record")
    ...
    async def playrecord(interaction):
        ...
        try:
            if vc and vc.is_connected():
                ...
                vc = await target_channel.connect(timeout=30.0, reconnect=False)
        except asyncio.TimeoutError:
            await interaction.edit_original_response(content="Voice connection timed out.")
            return
        except Exception as err:
            code = getattr(err, 'code', None)
            if code is not None:
                await interaction.edit_original_response(
                    content=f"Voice connect failed (close code {code}). This is usually a Discord voice session issue; restart the bot and retry."
                )
            else:
                await interaction.edit_original_response(content=f"Voice connect failed: {err}")
            return

        try:
            play_record(vc)
            await interaction.edit_original_response(content="Playing record output")
        except Exception as err:
            await interaction.edit_original_response(content=f"Connected, but audio failed: {err}")
    ...
    async def stoprecord(interaction):
        ...
        try:
            ...
            await interaction.response.send_message(f"Error stopping record output: {err}")
        else:
